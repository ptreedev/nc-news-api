...
exports.selectArticles = (sort_by, order, topic) => {
    const topicQuery = "SELECT slug FROM topics"
    const columnQuery = "SELECT articles.* , COUNT(comments.article_id) AS comment_count FROM articles LEFT JOIN comments ON articles.article_id = comments.article_id GROUP BY articles.article_id"
    const promiseTopic = db.query(topicQuery)
    const promiseColumn = db.query(columnQuery)
    return Promise.all([promiseTopic, promiseColumn])
    .then((results) => {
        const validTopics = results[0].rows.map((row) => row.slug)
        const validColumns = Object.keys(results[1].rows[1])
